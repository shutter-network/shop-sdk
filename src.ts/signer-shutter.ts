...
export class SignerShutter extends JsonRpcSigner {
  ...
  async isShutterPaused(): Promise<boolean> {
    const keyperSet = new Contract(
      this.keyperSetManagerAddress,
      KeyperSetManager,
      this.provider,
    )
    const inbox = new Contract(this.inboxAddress, InboxAbi, this.provider)
    const result = await Promise.all([inbox.paused(), keyperSet.paused()]).then(
      ([inboxPaused, keyperSetPaused]) => {
  ...
  async getEonForBlock(block: number): Promise<number> {
    const keyperSetManager = new Contract(
      this.keyperSetManagerAddress,
      KeyperSetManager,
      this.provider,
    )
  ...
  async getEonKey(eon: number): Promise<string> {
    const keyBroadcastContract = new Contract(
  ...
  async encryptOriginalTx(
    _tx: TransactionRequest,
    inclusionBlock: number,
  ): Promise<[Uint8Array, BigNumberish]> {
    ...
    const dataBytes = getBytes(
      encodeRlp(dataForShutterTX as RlpStructuredDataish),
    )
  ...
  async _sendTransactionTrace(
    tx: TransactionRequest,
    inclusionWindow: number = 17,
    blockProvider?: any,
  ): Promise<any> {
    ...
    const [executionTx, gasLimitExecuteTx] = await this.encryptOriginalTx(
      tx,
      executionBlock,
    )
    const includeTx =
      await inbox.submitEncryptedTransaction.populateTransaction(
        executionBlock,
        executionTx,
        gasLimitExecuteTx,
        tx.from,
    ...
    return new Promise<[Uint8Array, Promise<any>, number]>(
      (resolve, reject) => {
